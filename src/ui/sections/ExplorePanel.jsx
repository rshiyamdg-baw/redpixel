...
import { PhaseAbout, PhaseWorks, PhaseContact, PhaseDeepVoid, PHASE_CONTENT } from './PhaseComponents'
...
export default function ExplorePanel() {
  ...
  // THE THEME DETECTOR (Fixed for 0-Indexing! Phase 3 = Index 2, Phase 4 = Index 3)
  const isVitrailTheme = displayPhase >= 2; 
  
  // Sleek, minimal theme colors
  const accentColorClass = isVitrailTheme ? "bg-amber-600" : "bg-red-500";
  const textAccentClass = isVitrailTheme ? "text-amber-500" : "text-red-400";
  const svgStroke = isVitrailTheme ? "#d97706" : "#ff0044"; 
  ...
  return (
    <div ref={containerRef}>
      ...
      <div className="fixed top-0 bottom-40 sm:bottom-0 left-0 right-0 z-[30] flex items-center justify-center sm:justify-end px-4 sm:px-6 sm:pr-[10%] pt-10 sm:pt-0 pointer-events-none">
          {/* Elegant Dark Glass Border */}
          <div ref={bgRef} className={`w-full max-w-lg h-[75vh] sm:h-[80vh] relative pointer-events-auto border-l border-t border-white/5 ${isVitrailTheme ? 'border-amber-600/30 bg-[#020000]/90' : 'border-red-500/50 bg-black/85'}`} style={{ clipPath: 'inset(0% 100% 0% 0%)' }}>
              <div className="absolute inset-0 backdrop-blur-xl overflow-hidden">
                  
                  {/* Subtle, beautiful void background patterns */}
                  {isVitrailTheme ? (
                      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top_right,#d9770615_0%,transparent_50%),radial-gradient(ellipse_at_bottom_left,#ff003310_0%,transparent_50%)]" />
                  ) : (
      ...
      <svg className="fixed inset-0 w-full h-full pointer-events-none z-[60]">
        <path ref={svgPathRef} d={pathDef} fill="none" stroke={svgStroke} strokeWidth="1.5" strokeLinejoin="miter" style={{ filter: `drop-shadow(0px 0px 5px ${svgStroke})` }} />
      </svg>
      ...
      <section className="fixed top-0 bottom-40 sm:bottom-0 left-0 right-0 z-[50] flex items-center justify-center sm:justify-end px-4 sm:px-6 sm:pr-[10%] pt-10 sm:pt-0 pointer-events-none">
        ...
        <div className={`w-full max-w-lg max-h-full flex flex-col text-left relative`}>
          ...
          <div className={`tech-line origin-right absolute -bottom-4 sm:-bottom-10 -left-2 sm:-left-4 w-12 h-[1px] ${accentColorClass} opacity-0 hidden sm:block`} />
