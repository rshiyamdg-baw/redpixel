...
import { BoxGeometry, MeshPhysicalMaterial, MeshStandardMaterial, DoubleSide, MeshDepthMaterial, RGBADepthPacking } from 'three'
...
export default function GlassShell() {
  ...
  const { geometry, material, depthMaterial } = useMemo(() => {
    ...
    // ====================================================================
    // MOBILE ARCHITECTURE: Fast, 9-Loop, Emissive Standard Material
    // ====================================================================
    if (isLowEnd) {
      ...
      mat.onBeforeCompile = (shader) => {
        ...
        shader.fragmentShader = `
           ...
           Vitrail getOrosi(vec3 localNormal, vec2 currentUv) {
              ...
              // 9-LOOP: Fast Bevel Detection (Replaced the 25-loop for mobile!)
              ...
              Vitrail v; v.color = gColor; v.lead = customLead; v.normalTilt = calculatedTilt;
              return v;
           }
           ${shader.fragmentShader}
        ...
        shader.fragmentShader = shader.fragmentShader
          ...
          .replace('#include <color_fragment>', `#include <color_fragment>\n diffuseColor.rgb = mix(orosi.color, vec3(0.02), orosi.lead);`)
          .replace('#include <roughnessmap_fragment>', `#include <roughnessmap_fragment>\n roughnessFactor = mix(0.1, 0.8, orosi.lead);`)
          .replace('#include <metalnessmap_fragment>', `#include <metalnessmap_fragment>\n metalnessFactor = mix(0.1, 1.0, orosi.lead);`)
          .replace('#include <emissivemap_fragment>', `#include <emissivemap_fragment>\n vec3 glow = orosi.color * 1.5; totalEmissiveRadiance += mix(glow, vec3(0.0), orosi.lead);`)
      }
      ...
      depthMat.customProgramCacheKey = () => 'vitrail_mobile_shadow_v1'
      return { geometry: geo, material: mat, depthMaterial: depthMat }
    } 

    // ====================================================================
    // DESKTOP ARCHITECTURE: Your Original 25-Loop Masterpiece (Restored!)
    // ====================================================================
    else {
      ...
      mat.onBeforeCompile = (shader) => {
        ...
        shader.fragmentShader = `
           ...
           Vitrail getOrosi(vec3 localNormal, vec2 currentUv) {
              ...
              // YOUR ORIGINAL 25-LOOP: Perfect Bevel Details
              float edgeDist = 100.0;
              for(int j=-2; j<=2; j++)
              for(int i=-2; i<=2; i++){
                  vec2 b = vec2(float(i), float(j));
      ...
      depthMat.customProgramCacheKey = () => 'vitrail_desktop_shadow_v1'
      return { geometry: geo, material: mat, depthMaterial: depthMat }
    }
  }, [isLowEnd])
